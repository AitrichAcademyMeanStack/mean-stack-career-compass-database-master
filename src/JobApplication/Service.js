...
const getalljobapplications = async (page, limit, sortorder) => {
  try {
    ...
    if (applications) {
      ...
      const sortOption =
        sortorder === "oldest" ? { datesubmitted: 1 } : { datesubmitted: -1 };
      ...
      if (result.length > 0) {
        ...
        throw new BadRequestError(
          "Error occurred in getting paginated job applications"
        );
      }
    }
  } catch (error) {
...
const getjobapplications = async (
  ...
  try {
    ...
    if (existingcompanyuser) {
      ...
      if (
        existingjobpost &&
        existingjobpost.company.companyId.toString() ===
          existingcompanyuser._id.toString()
      ) {
        ...
        const result = await Jobapplication.find({
          "job.JobpostId": existingjobpost._id,
        })
          .sort(sortOption)
          .skip((page - 1) * limit)
          .limit(limit)
        ...
        if (result.length > 0) {
          logger.info(
            "successfully geetting all job applications with company user id"
          );
          return result;
        } else {
          logger.error("error occured in getting all job applications");
          throw new BadRequestError(
            "error occured in getting all job applications"
          );
        }
      } else {
...
const getallapplications = async (seekerid, page, limit, sortorder) => {
  try {
    ...
    if (existingseeker) {
      ...
      const result = await Jobapplication.find({
        "applicant.seekerId": existingseeker._id,
      })
        .sort(sortOption)
        .skip((page - 1) * perpage)
        .limit(perpage)
...
const deleteapplication = async (seekerid, applicationid) => {
  try {
    ...
    if (existingseeker) {
      ...
      if (
        findjobapplication.applicant.seekerId.toString() ===
        existingseeker._id.toString()
      ) {
        const deletejobapplication = await Jobapplication.findOneAndDelete({
          _id: applicationid,
        });
        if (!deletejobapplication) {
          logger.error(
            "Error occurred in deleting job application with specific id"
          );
          throw new BadRequestError(
            "Error occurred in deleting job application with specific id"
          );
    ...
    if (existingseeker) {
      ...
      if (existingprofile) {
        ...
        throw new NotFoundError(
          "job seeker profile not found with specific id"
        );
      }
    } else {
      logger.error("job seeker not found with specific id");
